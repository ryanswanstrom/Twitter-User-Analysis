...
```{r}
# this is a function to run a model on the training and validation sets
# and to check against the validation set
# transform is a parameter to indicate a transform, 0 is no transform, 1 is log, 2 is raised to the .06 power, this is a KLUDGE
model_test_function = function(eq, transform=0) {
  # create the model with training data
  train = lm(eval(eq) , data=training_data)
  print('model on training data')
  print( summary(train) )
  
  # create the same model with the validation data, just to compare parameter values
  validation = lm(eval(eq) , data=validation_data)
  print('model on validation data')
  print( summary(validation) )
  
  # check how closely the model will predict the values in the validation set
  predicted_vals = predict(train, newdata=validation_data)
  
  # unwind the transformations
  if (transform == 1) {
    predicted_vals = exp(predicted_vals)
  } else if (transform == 2) {
    predicted_vals = predicted_vals^(1/.06)
  }
  # compare with actuals
  # notice the function for the prediction error
  prediction_error = sum(abs(predicted_vals - validation_data$num_of_followers))/length(predicted_vals)
  print(paste('The average prediction error is:', prediction_error))
...
```{r}
m1_error = model_test_function(num_of_followers ~ age + num_following + num_of_lists)
```

## Model 2: Linear Model with num_following and num_of_lists

```{r}
m2_error = model_test_function(num_of_followers ~ num_following + num_of_lists)
```

## Model 3: Linear Model with just num_of_lists

```{r}
m3_error = model_test_function(num_of_followers ~ num_of_lists)
```
